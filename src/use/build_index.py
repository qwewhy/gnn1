...
class ModelValidator:
    ...
    def validate_checkpoint(checkpoint_path: Path, config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # 创建模型实例以验证结构匹配
            model = MetricLearningGAT(encoder_config=config['model'])
...
class RobustQueryEngine:
    ...
    def _load_dataset(self) -> bool:
        """加载数据集"""
        try:
            # 确保数据路径相对于项目根目录 / Ensure data path is relative to project root
    ...
    def _load_model(self) -> bool:
        ...
        try:
            ...
            self.model = MetricLearningGAT(encoder_config=model_config).to(self.device)
            
            # 尝试加载最佳模型
            checkpoint_dir = self.project_root / self.config['training']['checkpoint_dir']
    ...
    def encode_pattern_batch(self, data_batch) -> Optional[torch.Tensor]:
        ...
        try:
            with torch.no_grad():
                return self.model.encoder(data_batch.to(self.device), input_type='pattern')
        except Exception as e:
...
def build_pattern_index_robust(config_path: str, force_rebuild: bool = False):
    ...
    # 2. 检查是否需要重建索引
    data_root = query_engine.project_root / query_engine.config['data']['root']
...
def verify_index_integrity(config_path: str) -> bool:
    ...
    try:
        ...
        # 确保路径相对于项目根目录 / Ensure path is relative to project root
        project_root = Path(__file__).parent.parent.parent
